    ...
    xml.urlset xmlns: "http://www.sitemaps.org/schemas/sitemap/0.9" do
      %w[en ru].each do |language|
        static_routes.each do |static_route|
          xml.url do
            xml.loc "#{base_url}#{static_route}?language=#{language}"
          ...
          if static_route == '/events'
            30.times.each do |d|
              xml.url do
                xml.loc "#{base_url}/events?dates=%5B\"#{Time.zone.now.to_date}\"%2C\"#{(Time.zone.now + (d + 1).day).to_date}\"%5D&language=#{language}"
                xml.lastmod Time.zone.now.iso8601
        ...
        Firm.active.each do |firm|
          xml.url do
            xml.loc "#{base_url}/firms/#{GraphqlSchema.id_from_object(firm)}?language=#{language}"
            xml.lastmod Time.zone.now.iso8601
            xml.changefreq 'weekly'
            xml.priority 0.7
          end
        ...
        Event.published.each do |event|
          xml.url do
            xml.loc "#{base_url}/events/#{GraphqlSchema.id_from_object(event)}?language=#{language}"
            xml.lastmod Time.zone.now.iso8601
            xml.changefreq 'weekly'
            xml.priority 0.7
          end
        ...
        Interest.all.each do |interest|
          xml.url do
            xml.loc "#{base_url}/events?interests=%5B\"#{interest.slug}\"%5D&language=#{language}"
            xml.lastmod Time.zone.now.iso8601
            xml.changefreq 'daily'
            xml.priority 0.7
          end
