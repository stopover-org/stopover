import React, { useState } from "react";
import styled from "styled-components";
import { v4 as uuidv4 } from "uuid";
import Input from "../../Input";
import Typography from "../../Typography";
import Row from "../../Row";
import Column from "../../Column";
import Checkbox from "../../Checkbox";
import Popover from "../../Popover";
import Link from "../../Link";
import {
  InputSizes,
  CheckboxType,
  TypographySize,
  TypographyTags,
} from "../../StatesEnum";

const InputWrapper = styled.div`
  padding-top: 10px;
  padding-right: 10px;
  padding-bottom: 10px;
`;

const InputPadding = styled.div`
  padding-bottom: 10px;
`;

const InputNamePadding = styled.div`
  padding-right: 10px;
`;

const CheckboxWrapper = styled(Column)`
  padding: 10px;
`;

const CheckboxPadding = styled.div`
  padding-top: 10px;
  padding-bottom: 10px;
`;

const TypographyPadding = styled.div`
  padding-left: 10px;
`;

const LinkPadding = styled.div`
  padding-right: 10px;
`;

type AdditionalOptions = {
  text: string;
  price: string;
  currency: string;
};
type AllreadyInPrice = {
  text: string;
  price: string;
  currency: string;
};

type Props = {
  additionalOptions?: AdditionalOptions[];
  allreadyInPrice?: AllreadyInPrice[];
};

export const Forma = ({ additionalOptions, allreadyInPrice }: Props) => {
  const [checkbox, setCheckbox] = useState(false);
  return (
    <>
      <Row>
        <InputWrapper>
          <Row>
            <InputPadding>
              <InputNamePadding>
                <Input
                  label={
                    <Typography
                      size={TypographySize.MEDIUM}
                      as={TypographyTags.MEDIUM}
                    >
                      name
                    </Typography>
                  }
                  size={InputSizes.MEDIUM}
                />
              </InputNamePadding>
            </InputPadding>
            <InputPadding>
              <Input
                label={
                  <Typography
                    size={TypographySize.MEDIUM}
                    as={TypographyTags.MEDIUM}
                  >
                    surname
                  </Typography>
                }
                size={InputSizes.MEDIUM}
              />
            </InputPadding>
          </Row>
          <InputPadding>
            <Input
              label={
                <Typography
                  size={TypographySize.MEDIUM}
                  as={TypographyTags.MEDIUM}
                >
                  E-mail
                </Typography>
              }
              size={InputSizes.MEDIUM}
            />
          </InputPadding>
          <InputPadding>
            <Input
              label={
                <Typography
                  size={TypographySize.MEDIUM}
                  as={TypographyTags.MEDIUM}
                >
                  phone number
                </Typography>
              }
              size={InputSizes.MEDIUM}
            />
          </InputPadding>
        </InputWrapper>
        <CheckboxWrapper alignItems="start" justifyContent="start">
          <Typography size={TypographySize.LARGE} as={TypographyTags.LARGE}>
            Дополнительные опции
          </Typography>
          {additionalOptions &&
            additionalOptions.map((item, index) => (
              <CheckboxPadding key={index}>
                <Checkbox
                  id={uuidv4()}
                  animate
                  checked={checkbox}
                  type={CheckboxType.CHECKBOX}
                  onClick={() => setCheckbox(!checkbox)}
                >
                  <Row>
                    <TypographyPadding>
                      <Typography
                        size={TypographySize.H4}
                        as={TypographyTags.H4}
                      >
                        {item.text}
                      </Typography>
                    </TypographyPadding>
                    <TypographyPadding>
                      <Typography
                        size={TypographySize.H4}
                        as={TypographyTags.H4}
                      >
                        {item.price}
                      </Typography>
                    </TypographyPadding>
                    <Typography size={TypographySize.H4} as={TypographyTags.H4}>
                      {item.currency}
                    </Typography>
                  </Row>
                </Checkbox>
              </CheckboxPadding>
            ))}

          <Typography size={TypographySize.LARGE} as={TypographyTags.LARGE}>
            Уже входит в стоимость
          </Typography>
          {allreadyInPrice &&
            allreadyInPrice.map((item, index) => (
              <CheckboxPadding key={index}>
                <Checkbox
                  id={uuidv4()}
                  animate
                  checked={checkbox}
                  type={CheckboxType.CHECKBOX}
                  onClick={() => setCheckbox(!checkbox)}
                >
                  <Row>
                    <TypographyPadding>
                      <Typography
                        size={TypographySize.H4}
                        as={TypographyTags.H4}
                      >
                        {item.text}
                      </Typography>
                    </TypographyPadding>
                    <TypographyPadding>
                      <Typography
                        strikeThrough
                        size={TypographySize.H4}
                        as={TypographyTags.H4}
                      >
                        {item.price}
                      </Typography>
                    </TypographyPadding>
                    <Typography
                      strikeThrough
                      size={TypographySize.H4}
                      as={TypographyTags.H4}
                    >
                      {item.currency}
                    </Typography>
                  </Row>
                </Checkbox>
              </CheckboxPadding>
            ))}
        </CheckboxWrapper>
      </Row>
      <Row container alignItems="end">
        <LinkPadding>
          <Link href="intrests/cancel">
            <Typography
              color="#BE0000"
              size={TypographySize.MEDIUM}
              as={TypographyTags.MEDIUM}
            >
              Отмена бронирования
            </Typography>
          </Link>
        </LinkPadding>
        <Popover
          width="326px"
          isOpen={checkbox}
          component={
            <Typography size={TypographySize.SMALL} as={TypographyTags.SMALL}>
              Бесплатная отмена бронирования до 12:00 10 февраля <br />
              При отмене бронирования до 12:00 12 февраля штраф $50 <br />
              При отмене бронирования после 12:00 12 февраля отменить нельзя{" "}
              <br />
            </Typography>
          }
          onOpen={() => setCheckbox(true)}
          onClose={() => setCheckbox(false)}
        >
          <Row alignItems="end">
            <Typography size={TypographySize.H5} as={TypographyTags.H5}>
              Условия отмены бронирования
            </Typography>
          </Row>
        </Popover>
      </Row>
    </>
  );
};
export default React.memo(Forma);
